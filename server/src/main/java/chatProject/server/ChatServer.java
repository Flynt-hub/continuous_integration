...
public class ChatServer<T> implements UserAlgo, ChatroomAlgo<T>, MessageAlgo<T>, AutoCloseable {
    ...
    public UserInfo login(String userName) {
        final UserInfo user = new UserInfo(
                findUser(userName).orElse(new UserAccount(getUsers().size()-1, userName)),
    ...
    public int addChatroom(String chatroomName, UserInfo owner) {
        ...
        for ( Chatroom<T> chatroom : chatInstance.getCurentChatrooms() )
        {
            nameAvailable = ! newChatroom.getName().equals( chatroom.getName() );
        }
